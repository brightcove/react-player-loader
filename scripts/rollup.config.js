...
const primedPlugins = {
  ...
  replaceDev: replace({'process.env.NODE_ENV': JSON.stringify('production')}),
  replace: replace({'process.env.NODE_ENV': JSON.stringify('development')})
};

/* General Globals */
const moduleName = 'ReactPlayerLoader';
const pluginName = 'react-player-loader';
...
if (!isWatch) {
  ...
  minPlugins.splice(plugins.umd.indexOf(primedPlugins.replaceDev), primedPlugins.replace);
